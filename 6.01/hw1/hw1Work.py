...
def tokenize(string):
    tokens = string
    for sep in seps:
        tokens = tokens.replace(sep, ' %s ' % sep)
    return tokens.split()
    # return string
...
def parse(tokens):
    def parseExp(index):
        ...
        if numberTok(token):
            ...
            (leftTree, opIndex) = parseExp(index+1)
            op = tokens[opIndex]
            (rightTree, parIndex) = parseExp(opIndex+1)
            if op == '+':
                return Sum(leftTree, rightTree), parIndex+1
            elif op == '*':
                return Prod(leftTree, rightTree), parIndex+1
            elif op == '/':
                return Quot(leftTree, rightTree), parIndex+1
            elif op == "-":
                return Diff(leftTree, rightTree), parIndex+1
            elif op == "=":
                return Assign(leftTree, rightTree), parIndex+1
            else:
                raise Exception, "parseExp: operator is not valid"        
